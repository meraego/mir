...
<form action="<?php echo JRoute::_(JFactory::getURI()->toString()); ?>" method="post" name="adminForm">
    ...
    <table class="table table-striped">
        ...
        <tr>
            ...
            <?php if($this->params->get('show_status')) : ?>
                <th width="5%">
                    <?php echo JText::_('COM_MIR_STATUS'); ?>
                </th>
            <?php endif; ?>
            ...
            <?php if($this->params->get('show_num_options')): ?>
                <th width="5%">
                    <?php echo JText::_('COM_MIR_OPTIONS'); ?>
                </th>
            <?php endif; ?>
        </tr>
        </thead>
        ...
        for ($i = 0; $i < $n; $i++)
        {
            ...
            if(isset($menu_items))
            {
                ...
                {
                    if((@$item->query['view'] == 'idea') && (@$item->query['id'] == $row->id))
                    {
            ...
            $link = JRoute::_('index.php?option=com_mir&view=idea&id=' . $row->slug . $itemid);
            ?>
            <tr>
                <td>
                ...
                <?php if($this->params->get('show_status')) : ?>
                    <td>
                        <?php if($this->params->get('show_status_as')) : ?>
                            <img src="<?php echo JURI::base(); ?>media/mir/images/idea-<?php echo $row->status; ?>.gif"/>
                        <?php else: ?>
                            <?php echo JText::_('COM_MIR_' . $row->status); ?>
                        <?php endif; ?>
                    </td>
                <?php endif; ?>
                <?php if($this->params->get('show_num_voters')) : ?>
    ...
    <input type="hidden" name="view" value="idei"/>
