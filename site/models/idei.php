...
class JUIdeiModelIdei extends JModelLegacy
{
    var $_query = null;
    var $_data = null;
    var $_total = null;
    ...
    function _buildViewQuery()
    {
        if(empty($this->_query))
        {
			...
			CASE WHEN publish_down>NOW() THEN 'active' ELSE 'ended' END AS status,"
                . " (SELECT COUNT(v.id) FROM #__mir_votes AS v WHERE v.idea_id = m.id) AS voters, COUNT(o.id) AS numoptions "
                . " FROM #__mir_idei AS m "
                . " LEFT JOIN #__mir_options AS o "
                . " ON o.idea_id = m.id "
    ...
    {
        $filter_order     = $this->app->getUserStateFromRequest($this->option . '.idei.filter_order', 'filter_order', 'm.publish_down', 'cmd');
        $filter_order_Dir = $this->app->getUserStateFromRequest($this->option . '.idei.filter_order_Dir', 'filter_order_Dir', 'DESC', 'cmd');
    ...
    {
        $db               = JFactory::getDBO();
        $filter_order     = $this->app->getUserStateFromRequest($this->option . '.idei.filter_order', 'filter_order', 'm.id', 'cmd');
        $filter_order_Dir = $this->app->getUserStateFromRequest($this->option . '.idei.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        $search           = $this->app->getUserStateFromRequest($this->option . '.idei.search', 'search', '', 'string');
